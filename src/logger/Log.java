...
public final class Log
{
    ...
    {
        createLogger(false, "[timestamp] [type] [caller_class]: msg", "hh:mm:ss");
    }

    public static void createLogger(boolean debug)
    {
        createLogger(debug, "[timestamp] [type] [caller_class]: msg", "hh:mm:ss");
    }
    ...
    public static <T> void log(LogType type, T msg, int depth)
    {
        ...
        if (type.output instanceof LoggerPrintStream)
        {
            ((LoggerPrintStream) type.output).outputln(out);
        } else
        {
    ...
    public static <T> void log(LogType type, T msg)
    {
        log(type, msg, 0);
    }

    static <T> void out(T msg)
    {
        log(LogType.STD_OUT, msg);
    }

    static <T> void err(T msg)
    {
        log(LogType.STD_ERR, msg);
    }
    ...
    public static final class ClassGetter
    {
        ...
        public static String getCallerClassName(int depth)
        {
            ...
            {
                element = elements[BASE_DEPTH + 2 + depth];
            } else if (element.getClassName().startsWith("java.lang.Throwable"))
            {
                element = elements[BASE_DEPTH + 4 + depth];
            }
