...
public class PrintFormat
{
    ...
    public PrintFormat(String format, String timeFormat)
    {
        ...
        for (String spl : timeFormat.split(":"))
        {
            if (spl.contains("M"))
            {
                timeOrganizer[0] = spl.length();
            } else if (spl.contains("s"))
            {
                timeOrganizer[1] = spl.length();
            } else if (spl.contains("m"))
            {
                timeOrganizer[2] = spl.length();
            } else if (spl.contains("h"))
            {
                timeOrganizer[3] = spl.length();
            } else if (spl.contains("d"))
            {
                timeOrganizer[4] = spl.length();
            }
        }
        timer = new Timer();
    }

    public <T> String getPrintString(String type, String caller, T msg)
    {
        return MSG.matcher(CALLER_CLASS.matcher(TYPE.matcher(TIMESTAMP.matcher(format).replaceAll(Matcher.quoteReplacement(timer.getTime()))).replaceAll(Matcher.quoteReplacement(type))).replaceAll(Matcher.quoteReplacement(caller))).replaceAll(Matcher.quoteReplacement(String.valueOf((Object) msg)));
    }
    ...
    protected class Timer
    {
        ...
        protected String formatTime()
        {
            ...
            for (int i = 0; i < timeOrganizer.length; ++i)
            {
                ...
                if (times[i] == lastTimes[i])
                {
                    out.append(stringTimes[i]);
                } else
                {
